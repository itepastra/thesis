...
def ga_qas(
    ...
    for generation in tqdm(range(settings.generations), desc="Generations", leave=False):
        ...
        for _ in tqdm(range(settings.offsprint_size), desc="Generating offspring", position=1, leave=False):
